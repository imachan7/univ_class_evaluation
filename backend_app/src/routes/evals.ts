...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const fields = { attendance, assignments, exam_difficulty, clarity, interest, easy_credit };
    for (const [key, val] of Object.entries(fields)) {
      const n = Number(val);
      if (!Number.isInteger(n) || n < 1 || n > 5) {
        res.status(400).json({ message: `${key} は 1〜5 の整数で入力してください` });
        return;
      }
    }

    const lecture = await prisma.lecture.findUnique({ where: { id: lecture_id } });
    if (!lecture) {
    ...
    if (existing) {
      res.status(409).json({ message: 'この講義にはすでに評価を投稿済みです。編集してください。' });
      return;
    }
  ...
  try {
    ...
    const updated = await prisma.lecEval.update({
      ...
      data: {
        ...
        ...(exam_difficulty !== undefined && { exam_difficulty: Number(exam_difficulty) }),
