...
if (!lectureId) {
    ...
    evalListEl.innerHTML = '<p class="error">URLに id がありません。例: review.html?id=12</p>';
} else {
    backLink.href = `class_info.html?id=${lectureId}`;
    loadPage();
}
...
async function loadPage() {
    try {
        ...
        console.error(error);
        evalListEl.innerHTML = '<p class="error">データの取得に失敗しました。</p>';
    }
}
...
async function loadEvals() {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        evalListEl.innerHTML = '<p class="empty">まだ評価がありません。</p>';
        return;
    }
